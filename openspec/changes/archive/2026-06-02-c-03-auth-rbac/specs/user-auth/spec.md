...
The JWT access token payload MUST contain only `sub` (user UUID), `tenant_id`, `roles` (list of role name strings), `exp`, `iat`, and `jti`. Permissions MUST NOT be embedded in the token.
