...
### Frontend — diseño antes que código
17. **Frame de Stitch antes de codear cada pestaña del front.** Ninguna pestaña/página del frontend se empieza a implementar sin un frame de referencia de **Google Stitch** aprobado previamente. El frame define layout, jerarquía visual y sistema de diseño de esa pantalla; es el insumo de entrada del componente, no un extra. Aplica a CADA pestaña/página nueva — una por una, no en lote. Sin frame de referencia → no se escribe el componente. Es contrato, no sugerencia.
...
5. (SOLO frontend) Frame de Stitch de referencia por cada pestaña  → aprobado ANTES de codear (regla dura 17)
6. Implementar las tasks (cargando skills, Strict TDD)             → respetando las reglas duras
7. /opsx:archive C-NN-nombre + marcar [x] en CHANGES.md            → cerrar el change
```
